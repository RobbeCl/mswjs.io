...
Providing the `ctx.body()` utility function with a [`SourceBuffer`](https://developer.mozilla.org/en-US/docs/Web/API/SourceBuffer) object will use that buffer as the mocked response's body. Support of binary data allows to send any kind of media content (images, audio, documents) in a mocked response.
